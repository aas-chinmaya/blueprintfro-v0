...
const CreateTaskForm = ({ project_id, projectName, teamId, project }) => {
  ...
  const customSelectStyles = {
    ...
    option: (provided, state) => ({
      ...provided,
      backgroundColor: state.isSelected ? '#eff6ff' : '#ffffff',
      color: '#111827',
      cursor: 'pointer',
      '&:hover': {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="flex flex-col items-center">
          ...
          <p className="text-gray-600 text-sm font-medium">Loading teams...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="w-full max-w-2xl bg-white rounded-2xl shadow-lg p-6 sm:p-8">
        ...
        {taskStatus === 'loading' ? (
          <div className="flex flex-col items-center justify-center min-h-[16rem]">
            <div className="animate-spin rounded-full h-10 w-10 border-t-2 border-b-2 border-blue-600 mb-3"></div>
            <p className="text-gray-600 text-sm font-medium">Creating task...</p>
          </div>
        ) : (
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              {[
                ...
                <div key={field.name} className="flex flex-col">
                  <label className="text-sm font-medium text-gray-700 mb-1.5">{field.label}</label>
                  <div className="relative">
                    {field.type === 'select' && field.name === 'teamId' ? (
                      <Select
                        key={teams.map((t) => t.teamId).join('-')}
                      ...
                      <select
                        ...
                        value={formData[field.name]}
                        onChange={handleChange}
                        className="w-full p-3 pl-10 border border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 bg-white text-gray-700 placeholder-gray-400 hover:border-blue-300 transition-all duration-200"
                        required={field.required}
                      >
                        {field.options.map((option) => (
            ...
            <div className="flex flex-col">
              ...
              <div className="relative">
                <textarea
                  ...
                  placeholder="Task description"
                  rows={4}
                  className="w-full p-3 pl-10 border border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 bg-white text-gray-700 placeholder-gray-400 hover:border-blue-300 transition-all duration-200 resize-none"
                />
            ...
            <div className="flex justify-end gap-4 pt-4">
              ...
              <button
                type="submit"
                disabled={taskStatus === 'loading'}
                className="px-5 py-2.5 bg-blue-600 text-white text-sm font-medium rounded-lg hover:bg-blue-700 focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200"
              >
                {taskStatus === 'loading' ? (
                  <div className="flex items-center">
                    <div className="animate-spin rounded-full h-5 w-5 border-t-2 border-white mr-2"></div>
